...
class CircleLineCarryView(ctx : Context) : View(ctx) {
    ...
    data class CLCNode(var i : Int, val state : State = State()) {

        private var next : CLCNode? = null
        private var prev : CLCNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawCLCNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : CLCNode {
            var curr : CLCNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class CircleLineCarry(var i : Int, val state : State = State()) {

        private var curr : CLCNode = CLCNode(0)
